...
class SDE(ABC):
    ...
    def probability_ode(self, score_fn, batch, patch_len):
        return ProbabilityFlowODE(self, score_fn, batch, patch_len)


class ProbabilityFlowODE:
    def __init__(self, sde: SDE, score_fn=None, batch=None, patch_len=None):
        self.sde = sde
    ...
    def coefficients(self, x, t, z=None):
        # drift, diffusion = self.sde.coefficients(x.reshape(-1,x.shape[-1]), t.reshape(-1,t.shape[-1]))
        drift, diffusion = self.sde.coefficients(x, t)
        # print("drift",drift.shape)
        # print("diffusion",diffusion.shape)
        # print("get drift, diffusion")
        score_fn = self.score_fn(x.reshape(self.b,self.l,-1), t.reshape(self.b, self.l, -1), z)
        # print("score_fn", score_fn.shape)
        ...
        # print("func coefficients done")
        # print("ode_drift", ode_drift.shape)
